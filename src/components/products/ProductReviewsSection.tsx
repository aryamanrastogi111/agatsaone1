...
export function ProductReviewsSection({ reviews, heading = "What users are saying" }: ProductReviewsSectionProps) {
  ...
  }, [filter, showAll, reviews]);

  const totalCount = reviews.length;
  const avgRating = (reviews.reduce((s, r) => s + r.rating, 0) / totalCount).toFixed(1);
  ...
  return (
    <section className="py-12 bg-background">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        {!showAll && (filter === "all" ? reviews.length : reviews.filter(r => r.rating === filter).length) > 6 && (
