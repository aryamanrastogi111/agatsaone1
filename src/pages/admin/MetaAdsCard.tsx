...
function MetaAiAnalysisPanel({
  ...
  const keyMetrics = ai?.keyMetrics ?? [];
    ...
    <div className="border border-gray-100 rounded-lg overflow-hidden bg-white shadow-sm">
      <div className="flex items-center justify-between px-4 py-2.5 bg-gradient-to-r from-purple-50 to-pink-50 border-b border-gray-100">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-gray-800">
            <Sparkles size={14} className="text-purple-600" /> AI Corrective Actions
          </div>
          <p className="text-[10px] text-gray-500 mt-0.5">Detailed diagnosis from Meta delivery, site attribution, campaigns and creatives</p>
        </div>
        <button onClick={onRun} disabled={aiLoading || disabled}
          className="text-xs bg-purple-600 text-white px-3 py-1.5 rounded-md hover:bg-purple-700 disabled:opacity-50 shrink-0">
      ...
      {!ai && !aiLoading && !aiError && (
        <div className="p-6 text-xs text-gray-500 bg-gray-50">
          AI actions will appear here automatically. Use <span className="font-semibold text-purple-700">Refresh AI</span> if the latest data has just changed.
        </div>
      )}
      {ai && (
        <div className="p-5 space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2">
                {ai.targetScorecard.map((t, i) => (
                  <div key={`${t.metric}-${i}`} className={`rounded-lg p-3 border ${
          ...
          {keyMetrics.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {keyMetrics.map((m, i) => (
                  <div key={`${m.label}-${i}`} className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                    <div className="flex justify-between items-start mb-1">
          ...
          {ai.campaignActions && ai.campaignActions.length > 0 && (
            <div>
              <h4 className="text-xs font-bold text-gray-700 uppercase mb-3">Campaign Verdicts & Next Steps</h4>
              <div className="space-y-2">
                {ai.campaignActions.map((c, i) => {
                  ...
                  return (
                    <div key={`${c.name}-${i}`} className="border border-gray-100 rounded-lg p-3 bg-white flex items-start justify-between gap-3">
          ...
          {ai.creativeAnalysis && ai.creativeAnalysis.length > 0 && (
            <div>
              <h4 className="text-xs font-bold text-gray-700 uppercase mb-3 flex items-center gap-1"><Sparkles size={13} className="text-purple-600"/> Creative Analysis</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {ai.creativeAnalysis.map((c, i) => {
                  ...
                  return (
                    <div key={`${c.adName}-${i}`} className="border border-gray-100 rounded-lg p-3 bg-white flex gap-3">
                      {thumb ? (
                        <img src={thumb} alt="Ad creative thumbnail" className="w-16 h-16 rounded object-cover shrink-0 border border-gray-100" />
                      ) : (
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {ai.budgetReallocation.moves.map((m, i) => (
                  <div key={`${m.from}-${m.to}-${i}`} className="text-[11px] text-gray-700 bg-emerald-50/50 border border-emerald-100 rounded px-3 py-2">
            ...
            <div>
              ...
              <div className="space-y-2">
                {ai.actionPlan7Day.map((a, i) => (
                  <div key={`${a.day}-${i}`} className="border-l-4 border-blue-500 bg-blue-50/40 pl-3 pr-3 py-2 rounded-r">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {recommendations.map((r, i) => (
                      <div key={`${r.action}-${i}`} className="bg-white border border-gray-100 rounded-lg p-3 text-xs">
              ...
              {alerts.length > 0 && (
                <div>
                  ...
                  <div className="space-y-2">
                    {alerts.map((a, i) => (
                      <div key={`${a}-${i}`} className="flex items-start gap-2 text-xs text-rose-700 bg-rose-50 p-2 rounded border border-rose-100">
