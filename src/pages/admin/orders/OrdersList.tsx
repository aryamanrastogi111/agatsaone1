...
function RazorpayOrdersTab() {
  ...
  const fetchOrders = async () => {
    ...
    if (search)
      query = query.or(
        `customer_email.ilike.%${search}%,customer_name.ilike.%${search}%,razorpay_order_id.ilike.%${search}%`
      );
...
/* ════════════════════════════════════════
   Shopify (legacy) orders tab — unchanged
════════════════════════════════════════ */
      ...
      <div className="flex gap-1 bg-gray-900 border border-gray-800 rounded-xl p-1 w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
            tab === "razorpay" ? "bg-blue-600 text-white" : "text-gray-400 hover:text-white"
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
            tab === "shopify" ? "bg-green-600 text-white" : "text-gray-400 hover:text-white"
          }`}
        >
